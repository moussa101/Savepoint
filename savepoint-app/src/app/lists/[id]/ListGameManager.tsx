      ...
      {orderedItems.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-sm)', marginBottom: 'var(--space-lg)' }}>
          ...
          {orderedItems.map((item, index) => (
            <div key={item.id} className="card" style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-md)', padding: 'var(--space-sm) var(--space-md)' }}>
              <span style={{ width: 24, textAlign: 'center', color: 'var(--text-muted)', fontWeight: 700 }}>{index + 1}</span>
              <span style={{ flex: 1, fontWeight: 600, fontSize: 'var(--text-sm)' }}>{item.game.name}</span>
